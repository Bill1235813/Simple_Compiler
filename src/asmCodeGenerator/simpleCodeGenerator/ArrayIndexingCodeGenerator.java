...
public class ArrayIndexingCodeGenerator implements SimpleCodeGenerator {
    ...
    public ASMCodeFragment generate(ParseNode node) {
        ...
        // [... array index]
        storeITo(frag, ARRAY_INDEXING_INDEX);
        storeITo(frag, ARRAY_INDEXING_ARRAY);

        // [...]
        loadIFrom(frag, ARRAY_INDEXING_ARRAY);
        frag.add(JumpFalse, RunTime.NULL_ARRAY_RUNTIME_ERROR);
        loadIFrom(frag, ARRAY_INDEXING_INDEX);
        frag.add(JumpNeg, RunTime.NEGATIVE_LENGTH_ARRAY_RUNTIME_ERROR);

        loadIFrom(frag, ARRAY_INDEXING_INDEX); // [... index]
        loadIFrom(frag, ARRAY_INDEXING_ARRAY); // [... index array]
        ...
        loadIFrom(frag, ARRAY_INDEXING_ARRAY); // [... array]
        frag.add(PushI, Record.ARRAY_HEADER_SIZE);
        frag.add(Add); // [... start_addr]

        loadIFrom(frag, ARRAY_INDEXING_INDEX); // [... start_addr index]
